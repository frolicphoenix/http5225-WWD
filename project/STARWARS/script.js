...
let planetEl = document.querySelector(".planet");
let iconStarWarsEl = document.querySelector(".icon-star-wars");
let descriptionStarWarsEl = document.querySelector(".description-star-wars");
let temperatureStarWarsEl = document.querySelector(".temp-star-wars");
let searchBarStarWars = document.querySelector(".search-bar-star-wars");
let searchStarWarsEl = document.querySelector(".star-wars .search button");
let weatherStarWarsEl = document.querySelector(".star-wars .weather");
let funnyForecastStarWarsEl = document.querySelector(".funny-forecast-star-wars");
...
let starWarsPlanets = {
    "Tatooine": { "temp_c": 40, "description": "Desert", "icon": "//cdn.weatherapi.com/weather/64x64/day/116.png" },
    "Hoth": { "temp_c": -50, "description": "Frozen", "icon": "//cdn.weatherapi.com/weather/64x64/day/176.png" },
    "Endor": { "temp_c": 25, "description": "Forest", "icon": "//cdn.weatherapi.com/weather/64x64/day/176.png" },
    "Kashyyyk": { "temp_c": 30, "description": "Jungle", "icon": "//cdn.weatherapi.com/weather/64x64/day/176.png" },
    "Coruscant": { "temp_c": 20, "description": "Urban", "icon": "//cdn.weatherapi.com/weather/64x64/day/116.png" },
    "Naboo": { "temp_c": 15, "description": "Lake", "icon": "//cdn.weatherapi.com/weather/64x64/day/356.png" }
};

let weather = {
    "apikey": "3d6c8d17581d485fb3f04952241906",
    fetchWeather: function (city) {
        fetch("http://api.weatherapi.com/v1/current.json?key=3d6c8d17581d485fb3f04952241906&q=" + city + "&aqi=no")
    ...
    displayWeather: function (data) {
        ...
        const randomForecast = Object.keys(funnyForecasts)[Math.floor(Math.random() * Object.keys(funnyForecasts).length)];
        funnyForecastEl.innerText = funnyForecasts[randomForecast];
...
let starWarsWeather = {
    fetchWeather: function (planet) {
        const data = starWarsPlanets[planet];

        planetEl.innerText = `Weather in ${planet}`;
        iconStarWarsEl.src = data.icon;
        descriptionStarWarsEl.innerText = data.description;
        temperatureStarWarsEl.innerText = `Temperature: ${data.temp_c}°C`;

        const randomForecastStarWars = Object.keys(funnyForecasts)[Math.floor(Math.random() * Object.keys(funnyForecasts).length)];
        funnyForecastStarWarsEl.innerText = funnyForecasts[randomForecastStarWars];

        weatherStarWarsEl.classList.remove("loading");
        document.body.style.backgroundImage = "url('https://source.unsplash.com/1600x900/?" + planet + "')";
    },
    search: function () {
        this.fetchWeather(searchBarStarWars.value);
    }
};
...
searchStarWarsEl.addEventListener("click", () => {
    starWarsWeather.search();
});

searchBarStarWars.addEventListener("keyup", (event) => {
    if (event.key === "Enter") {
        starWarsWeather.search();
    }
});
...
starWarsWeather.fetchWeather("Tatooine");
